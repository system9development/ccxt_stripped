...
import { ExchangeError, ArgumentsRequired, InvalidNonce, BadSymbol } from './base/errors';
import { DECIMAL_PLACES, TRUNCATE } from './base/functions/number';
...
export default class deepwaters extends Exchange {
    ...
    async fetchOrders (symbol = undefined, since = undefined, limit = undefined, params = {}) {
        ...
        if (since !== undefined) {
            // TRUNCATE, this.markets[symbol]['precision']['quote'], this.precisionMode, this.paddingMode
            since = Precise.stringMul (since, '1000');
            request['created-at-or-after-micros'] = since;
        }
        if (limit !== undefined) {
            limit = this.decimalToPrecision (limit, TRUNCATE, 4);
            request['limit'] = limit;
        }
    ...
    async fetchOrder (id, symbol = undefined, params = {}) {
        ...
        if (isVenueId) {
            response = await this.privateGetOrdersByVenueOrderIdId (this.extend (request, params));
        } else {
            response = await this.privateGetOrdersByCustomerObjectIdId (this.extend (request, params));
        }
        const success = this.safeValue (response, 'success', false);
        if (!success) {
            return this.handleError (response);
